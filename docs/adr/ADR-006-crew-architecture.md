...
- Any agent that signals a benign-empty outcome (researcher finds no signals; scorer below operator-tunable floor; checkpoint rejected) routes to a `terminate_no_draft` node. System-decided empty-outcomes (`score_gated`, `coverage_gated`) finalise the workflow as **COMPLETED** — the workflow ran successfully; the answer was "not worth drafting." Operator-decided empty-outcomes (checkpoint rejected) finalise as **CANCELLED** with `terminate_reason='rejected'` — this is the CheckpointSystem's terminal status transition (`reject_checkpoint`, Slice 5), preserved by the orchestrator rather than overridden. The split is intentional: CANCELLED records that an operator deliberately stopped an otherwise-valid workflow; COMPLETED records that the system itself decided the answer was empty. This distinction matters for the audit story: an empty-signals run is not a failure, and an operator rejection is distinguishable from a system gate. **(Amended 2026-05-15 after Slice 8 calibration surfaced the divergence between the original ADR prose and Slice 6's code: the code, which reflects Slice 5's CheckpointSystem contract, is the correct shape; the prose was written before Slice 5 was implemented.)**
...
**Rate-table change (2026-05-15, post-calibration).** During Phase 3 prep the Anthropic pricing page was re-verified and Claude Opus 4.7 was found to have been repriced from $15/$75 per MTok (the rate recorded in `llm.py` and assumed throughout Slice 8 calibration) to **$5/$25 per MTok** — a 3× reduction. The rate table in `llm.py` was updated on 2026-05-15; the calibration numbers in this addendum reflect the OLD (Slice 8) rates as billed at that time. Going forward, Opus-bearing runs will cost ~3× less than the Slice 8 range below until the next re-verification surfaces another change.

**Addendum — Slice 8 historical calibration (2026-05-15, OLD Opus rates of $15/$75 per MTok).** Four real-company runs against the unmodified ADR-004 placeholder rubric produced the first empirical cost distribution. Per-full-run total cost ranged **$0.066 – $0.171, mean $0.126**; the gated-no-drafter case (Anthropic, score 53.33 / floor 55.0) anchored the low end at $0.066, the retry-fired cases (Plaid persistent-fail, Modal Labs retry-success) anchored the high end at $0.167–$0.171. **The bounded-retry edge fired on 2 of 3 drafter-invoked runs (66%); of those, retry succeeded on 1 and failed on 1.** Per-agent typical costs: Researcher $0.023–$0.046 (well below the ADR-003 addendum's $0.067 estimate — that figure reflected a high-citation outlier, not the central tendency); Extractor $0.008–$0.013; Scorer $0.006–$0.010; Drafter initial $0.054–$0.063 (matches the $0.025–$0.060 ADR estimate at the upper bound); Critic ~$0.001 per Mode-2 call (0–3 calls per critic invocation depending on failure count). The upstream chain is therefore closer to **$0.04–$0.07 combined** in practice — about 3× the original §1 estimate of $0.01–$0.02. The checkpoint-rationale's break-even logic is unaffected: Opus dominates by a 2–4× factor and the savings calculus stands. The rubric was the unmodified ADR-004 template throughout this calibration — score numbers are structurally comparable across runs but not operationally tuned against any real ICP.

**Addendum — Current calibration (2026-05-15, NEW Opus rates of $5/$25 per MTok, Phase 3 public-push prep).** Two real-company runs against the same unmodified ADR-004 placeholder rubric, after the Opus 4.7 rate change. Both runs landed under a $0.15 per-run cost cap with healthy margin.

| Company | Score / Tier | Coverage | Drafter ran? | Critic outcome | Workflow status | Total cost |
|---|---|---|---|---|---|---|
| Anthropic | 76.00 / B | 50% | yes (initial + 1 retry) | initial: 4 failed claims (4 Mode-2 calls); retry: PASSED (0 Mode-2 calls — mechanical substring chain held end-to-end) | COMPLETED with draft | **$0.135** |
| Plaid     | 100.00 / A | 50% | yes (initial + 1 retry) | initial: 3 failed claims (3 Mode-2 calls); retry: 1 failed claim, classified `fixable_paraphrase` — the extractor's source span had truncated mid-word at `"accoun"` so the drafter's `"accounts"` legitimately failed substring match | **FAILED — halt-not-ship** (`terminate_reason=critic_failure`) | **$0.111** |

**Per-agent costs at new Opus rates (averaged across both runs):**
- Researcher (Haiku + web_search): **$0.048–$0.067** (2–3 web_searches; dominated by per-search fees, not LLM tokens — at new Opus pricing the researcher is now the **largest single line item** on most runs).
- Extractor (Sonnet): $0.010–$0.011 (matches Slice 8).
- Scorer (Sonnet): $0.007 (matches Slice 8).
- Drafter initial (Opus 4.7): **$0.020–$0.022** — about **2.7× cheaper than Slice 8's $0.054–$0.063** (matches the 3× rate reduction).
- Drafter retry (Opus 4.7): $0.023–$0.024 (same shape as initial; slightly higher because the retry prompt includes the critic's violation context).
- Critic Mode-2 (Haiku): ~$0.001 per call (matches Slice 8; rate unchanged).

**Findings of substance, on top of the cost recalibration:**

1. **Plaid halted again at current pricing.** The Slice 8 halt-not-ship outcome on Plaid (Tier A, drafter ran, retry failed, workflow refused to ship) replayed identically today. The bounded-retry edge is not a one-off behaviour; it is the framework operating as designed. **At new Opus rates this halt cost $0.111 — the price of one bounded refusal to ship dubious prose, with the full audit trail preserved.** This is the README's centerpiece example.

2. **Today's retry-fired-and-succeeded outcome was Anthropic** (Slice 8's analog was Modal Labs). The pattern is stable across companies and pricing regimes: when the drafter's first attempt fails the substring chain on stylistic edits (dropped suffixes, paraphrases), the retry with critic-violation context recovers on a measurable fraction of attempts.

3. **Cost bottleneck shifted.** Pre-reprice, the drafter dominated per-run cost (~40–50% of total). Post-reprice, the **researcher's web_search line items now dominate** ($0.048–$0.067 vs the drafter's $0.020–$0.024). The "approve before drafting" checkpoint still pays back on rejected leads (saving ~$0.04–$0.07 in drafter+critic costs per rejection), but the savings calculus is now closer to break-even on the gating cost than the 2–4× Slice 8 dominance — operators tuning daily budgets should plan accordingly.

4. **A new failure-classification observation.** Plaid's retry-failed claim was classified by the critic's Mode-2 LLM as `fixable_paraphrase` — and the explanation pointed to an **upstream extractor artifact** (a source-span quote truncated mid-word at `"accoun"`). The substring check correctly caught the miss; the framework correctly halted; the audit trail localises the root cause one upstream agent past the drafter. This is the first calibrated example of the contract surfacing an *extractor* issue via a *drafter* halt — a stronger story than "Opus hallucinated" because the failure is structurally traceable. Worth a future ADR if it recurs: should the extractor pad source-span boundaries to the next word boundary? — but **out of scope for Phase 3 (no code changes in the public-push slice)**.

The checkpoint break-even logic from §1 still stands at the qualitative level; the quantitative break-even point has shifted closer to parity post-reprice and would benefit from one more calibration cycle if a real ICP rubric replaces the placeholder.
