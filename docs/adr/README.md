...
| ADR-003 | [Web search tool for Researcher agent](ADR-003-web-search-tool.md) — Anthropic `web_search` server-side tool routed through `llm.py`; per-search fee aggregated into `LLMResult.cost_usd`; `search_result` content blocks captured verbatim into `RawSignals` so the fabrication-resistance substring check grounds against the exact text the model read | Accepted (2026-05-14) |
