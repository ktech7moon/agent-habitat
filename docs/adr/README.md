...
| ADR-006 | [Phase 2 crew architecture, `run_step()` extraction, fabrication-resistance contract](ADR-006-crew-architecture.md) — strictly sequential 5-agent pipeline with one human checkpoint before the drafter, single shared TypedDict state, `run_step()` context manager owned by `orchestration/`, critic + pure-Python substring check as the fabrication contract | Accepted (2026-05-14), Amended (2026-05-15) — rejection-path CANCELLED/COMPLETED prose aligned with code; Opus 4.7 rate-table change footnote |
| ADR-007 | [Retry / backoff policy for transient LLM failures](ADR-007-llm-retry-policy.md) — 3 attempts (1 initial + 2 retries), exponential backoff with jitter, Retry-After honoured on 429s, composes cleanly with ADR-006 §1's fabrication-retry edge | Accepted (2026-05-15) |
